...
public interface TaskRepository extends JpaRepository<TaskEntity, String> {
    ...
    @Query(value = "SELECT t.* FROM tasks t " +
            ...
            orderByPriority,
            nativeQuery = true)
    Slice<TaskEntity> findAllTasksWithoutFilters(@Param("userId") Integer userId, @Param("groupId") Integer groupId, Pageable page);

    @Query(value = "SELECT t.* FROM tasks t " +
            "join users u on t.reporter_id = u.id and u.group_id = :groupId " +
            "WHERE t.status = 'CANCELLED'", nativeQuery = true)
    Slice<TaskEntity> findAllClosedTasks(@Param("groupId") Integer groupId, Pageable page);

    @Query(value = "select t.* from tasks t " +
            "where t.status !='CANCELLED' and t.status !='COMPLETED' and t.reporter_id = :userId " +
            orderByPriority,
            nativeQuery = true)
    Slice<TaskEntity> findTasksWhereUserIsReporterAndTasksActive(@Param("userId") Integer userId, Pageable page);

    @Query(value = "select t.* from tasks t " +
            "where t.status ='COMPLETED'  and t.reporter_id = :userId " +
            orderByPriority,
            nativeQuery = true)
    Slice<TaskEntity> findTasksWhereUserIsReporterAndTasksCompleted(@Param("userId") Integer userId, Pageable page);
    ...
    @Query(value = "SELECT t.* FROM tasks t " +
            ...
            orderByPriority,
            nativeQuery = true)
    Slice<TaskEntity> findTasksWhereUserIsExecutorAndTasksActive(@Param("userId") Integer userId, Pageable page);
    ...
    @Query(value = "SELECT t.* FROM tasks t " +
            ...
            orderByPriority,
            nativeQuery = true)
    Slice<TaskEntity> findTasksWhereUserIsExecutorAndTasksCompleted(@Param("userId") Integer userId, Pageable page);
}